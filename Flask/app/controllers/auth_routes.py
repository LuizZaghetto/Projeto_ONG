...
@auth_routes_bp.route("/registro", methods=['GET'])
def registro():
...
@auth_routes_bp.route('/registro', methods=['POST'])
def registro_post():
    usuarioform = forms.registroForm()
    ONGform = forms.ONGregistroForm()
    form_type = request.form.get('form_type')
        ...
        if usuario is None:
            try:
                ...
                if avatar:
                    ...
                    avatar_filename = f"{slug_usuario}-{func.gerar_hash(avatar_f.filename)}"
                    avatar_path = os.path.join(
                        current_app.config['UPLOAD_FOLDER'], avatar_filename)
                    print(avatar_path)
                    avatar_f.save(avatar_path)
                usuario = models.Usuarios(
                    ...
                    slug=slug_usuario,
                    avatar_path=f"uploads/usuarios/{avatar_filename}" if avatar_filename else None
                )
